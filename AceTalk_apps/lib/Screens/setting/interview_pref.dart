...
class _InterviewPrefScreenState extends State<InterviewPrefScreen>
  ...
  Future<void> _save() async {
    ...
    showAppSnack(context, 'Interview preferences saved!', AppColors2.green, Icons.check_circle_rounded);
  }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [AppColors2.blue.withOpacity(0.2), AppColors2.purple.withOpacity(0.1)],
  ...
  Widget _chip(String label, String value, Color color) {
    return Column(children: [
      Text(label, style: GoogleFonts.dmSans(color: AppColors2.ts, fontSize: 10, fontWeight: FontWeight.w600)),
      ...
      body: Stack(children: [
        AnimatedBuilder(
          ...
          builder: (_, __) => CustomPaint(
            painter: BgPainter(_bgAnim.value, orbA: AppColors2.blue, orbB: AppColors2.purple),
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                Text('Interview Preferences',
                    style: GoogleFonts.spaceMono(fontSize: 16, color: AppColors2.tp, fontWeight: FontWeight.w700)),
              ]),
            ),
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  GlassCard(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      _prefRow('Type', Icons.category_rounded, AppColors2.blue,
                          ChipSelector(options: const ['HR','Technical','Behavioral','Mixed'], selected: _type, color: AppColors2.blue, onSelect: (v) => setState(() => _type = v))),
                      const SizedBox(height: 16),
                      Divider(color: Colors.white.withOpacity(0.06)),
                      const SizedBox(height: 16),
                      _prefRow('Format', Icons.question_answer_rounded, AppColors2.cyan,
                          ChipSelector(options: const ['MCQ','Descriptive','Voice','Mixed'], selected: _format, color: AppColors2.cyan, onSelect: (v) => setState(() => _format = v))),
                    ]),
                  ),
                  const SizedBox(height: 16),
                  ...
                  GlassCard(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      _prefRow('Difficulty Level', Icons.bar_chart_rounded, AppColors2.gold,
                          ChipSelector(options: const ['Beginner','Intermediate','Advanced','Expert'], selected: _difficulty, color: AppColors2.gold, onSelect: (v) => setState(() => _difficulty = v))),
                      const SizedBox(height: 16),
                      Divider(color: Colors.white.withOpacity(0.06)),
                      const SizedBox(height: 16),
                      _prefRow('Session Duration', Icons.schedule_rounded, AppColors2.green,
                          ChipSelector(options: const ['15 min','30 min','45 min','60 min'], selected: _duration, color: AppColors2.green, onSelect: (v) => setState(() => _duration = v))),
                      const SizedBox(height: 16),
                      Divider(color: Colors.white.withOpacity(0.06)),
                      const SizedBox(height: 16),
                      _prefRow('Time / Question: ${_questionTime}s', Icons.timer_rounded, AppColors2.cyan,
                          SliderTheme(
                            data: SliderThemeData(
                              activeTrackColor: AppColors2.cyan, inactiveTrackColor: AppColors2.s2,
                              thumbColor: AppColors2.cyan, overlayColor: AppColors2.cyan.withOpacity(0.15),
                              thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 9), trackHeight: 5,
                            ),
                            child: Slider(min: 30, max: 180, divisions: 10, value: _questionTime.toDouble(),
                                onChanged: (v) => setState(() => _questionTime = v.toInt())),
                          )),
                    ]),
                  ),
                  const SizedBox(height: 16),
                  ...
                  GlassCard(
                    child: Column(children: [
                      _toggleRow('Real-time Feedback',    'Instant AI hints after each answer',    Icons.bolt_rounded,     AppColors2.cyan, _realFeedback, () => setState(() => _realFeedback = !_realFeedback)),
                      Divider(color: Colors.white.withOpacity(0.06), height: 24),
                      _toggleRow('Mock Interview Mode',   'Simulates a real interview experience', Icons.videocam_rounded,  AppColors2.blue, _mockMode,     () => setState(() => _mockMode     = !_mockMode)),
                      Divider(color: Colors.white.withOpacity(0.06), height: 24),
                      _toggleRow('Company-Specific Mode', 'Questions from top companies',          Icons.business_rounded,  AppColors2.gold, _companyMode,  () => setState(() => _companyMode  = !_companyMode)),
                    ]),
                  ),
                  if (_companyMode) ...[
                    const SizedBox(height: 12),
                    GlassCard(
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text('Select Companies',
                            style: GoogleFonts.dmSans(color: AppColors2.ts, fontSize: 12, fontWeight: FontWeight.w600, letterSpacing: 0.4)),
                        const SizedBox(height: 12),
                        Wrap(
                          ...
                          children: _companies.map((c) {
                            ...
                            return GestureDetector(
                              onTap: () {
                                ...
                                setState(() { if (sel) _selectedCompanies.remove(c); else _selectedCompanies.add(c); });
                              },
                              child: AnimatedContainer(
                                duration: const Duration(milliseconds: 200),
                                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 7),
                                decoration: BoxDecoration(
                                  color: sel ? AppColors2.gold.withOpacity(0.15) : AppColors2.s2,
                                  borderRadius: BorderRadius.circular(10),
                                  border: Border.all(color: sel ? AppColors2.gold.withOpacity(0.5) : Colors.white12),
                                ),
                                child: Text(c, style: GoogleFonts.dmSans(
                                    color: sel ? AppColors2.gold : AppColors2.ts, fontSize: 12.5, fontWeight: FontWeight.w700)),
                              ),
                            );
                          }).toList(),
                        ),
